...
#include <vector>
#include <string>
#include <Libraries/Fonts/font_awesome_5.h>
#include <Libraries/Fonts/fa_solid_900.h>
#include <Libraries/Fonts/tahoma.h> // <-- Required font!
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			SetNextWindowPos(ImVec2(vars::screen_size.x - NOTIFY_PADDING_X, vars::screen_size.y - NOTIFY_PADDING_Y - height), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);

			// Here we render the toast content
			{
				PushTextWrapPos(vars::screen_size.x / 3.f); // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
